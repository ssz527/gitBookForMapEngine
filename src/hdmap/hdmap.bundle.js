  ...
  zoomChange: function zoomChange(e) {
    ...
    if (zoom % 1 === 0) {
      ...
      for (var item in layerArr) {
        ...
        var zl = lkey.split('_');
        if (zl[1]) {
          if (zoom >= parseInt(zl[1]) && layer.getVisibleFlag()) {
...
function optionsVerify(options) {
  ...
  if (!options.gisEngine) {
    ...
    if (!options.mapUrl) {
      console.warn('tilemap param without tile service url');
      return false;
    }
    if (!options.sizeH || !options.sizeW) {
      console.warn('tilemap param without extent of the map');
      return false;
    }
    if (!options.centerGPS) {
      console.warn('tilemap param without centerGPS');
      return false;
    }
    if (!options.maxZoom) {
      options.maxZoom = 19;
...
HDMap.prototype.initTileMap = function (options) {
  ...
  var mapMinZoom = options.minZoom;
  ...
  this._map.getView().setZoom(options.zoom);
};
...
HDMap.prototype.addMarker = function (markerInfo, styleObj, layerkey) {
  ...
  if (!markerInfo.id) {
    console.warn('addMarker Error : id is required');
    return null;
  }
  if (markerInfo.zoomLevel && (markerInfo.zoomLevel > this._map.getView().getMaxZoom() || markerInfo.zoomLevel < this._map.getView().getMinZoom())) {
    console.warn('addMarker Error : zoomLevel is wrong');
    return null;
  }
...
HDMap.prototype.addMarkerByGPS = function (markerInfo, styleObj) {
  if (this.mapConfig.scaleType === 1 || this.mapConfig.scaleType === '1') {
    ...
    console.warn('ScaleType is error');
  }
};
...
HDMap.prototype.addMarkersByGPS = function (markerList, styleObj) {
  if (!(markerList instanceof Array)) {
    console.warn('addMarkersByGPS require an Array instance argument');
    return;
  }
...
HDMap.prototype.addLayerByLayerKey = function (layerKey) {
  if (layerKey) {
    // var hdmap = this
    var zd = 11;
    var adarr = layerKey.split('_');
    var zdo = hdmap.commonConfig.getLayerZindex(adarr[0]);
    ...
    if (arr[1]) {
      ...
      var z = this._map.getView().getZoom();
      var flag = true;
...
HDMap.prototype.setLayerVisible = function (layerKey, flag, isforbi) {
  var z = this._map.getView().getMaxZoom();
  var nowZoom = this._map.getView().getZoom();
  var lk = '';
  var bool = false;
  var layer = null;
...
HDMap.prototype.transfromWGSToBitMap = function (lonlat) {
  ...
  if (this.mapConfig.scaleType === 1 || this.mapConfig.scaleType === '1') {
    ...
    console.warn('ScaleType must equal 1');
    return null;
  }
};
...
HDMap.prototype.warnMarkerStart = function (warnInfo, fn) {
  ...
  // 添加样式
  feature.setStyle(warnStyle(11, 'rgba(' + color + ',1)'));
  ...
  this._map.on('postcompose', function () {
    ...
    if (radius < 10) {
      ...
      feature1.setStyle(warnStyle(16, 'rgba(' + color + ',0.5)'));
    } else if (radius < 30 && radius >= 20) {
      feature2.setStyle(warnStyle(22, 'rgba(' + color + ',0.3)'));
    } else if (radius >= 30) {
      radius = 0;
      feature2.setStyle(warnStyle(0, 'rgba(' + color + ',0.3)'));
      feature1.setStyle(warnStyle(0, 'rgba(' + color + ',0.5)'));
...
function getFeaturesInExtent(map, coordinate) {
  ...
  if (map.mapConfig.gisEngine === 'tile') {
    ...
    if (resolution === 1) {
      ...
      distance = 100 * Math.log2(resolution);
    }
  } else {
